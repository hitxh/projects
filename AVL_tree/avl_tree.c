...
static void print_node(AVLTreeNode *node, char* sufix, char fromdir, void (*print_data)(AVLTreeNode *)) {
  ...
  if (node != NULL) {
    ...
    print_node(avl_tree_node_child(node, AVL_TREE_NODE_RIGHT), suf2, 'R', print_data);
		printf("%s  +-", sufix);
		print_data(node);
  	strcpy(suf2, sufix);
    ...
    print_node(avl_tree_node_child(node, AVL_TREE_NODE_LEFT), suf2, 'L', print_data);
    if (fromdir == 'R') 
      printf("%s\n", suf2);
    free(suf2);
  }
...
void avl_tree_print(AVLTree *tree, void (*print_fce)(AVLTreeNode *)) {
	if (tree != NULL)
		print_node(tree->root_node, "", 'X', print_fce);
}
