...
#	
# Description:
# Script highlights important traces from strace utility.
# Output of hltrace is in html format.
#	
#	What hltrace highlighting?:
...
usage() {
	printf "\
Strace output highlighter.

Usage: 
	hltrace [options] <record.strace >record.html

Options:
	-s SYSCALL  bold specified syscall\n"
  exit 0
}
...
while getopts ":hs:" opt; do
	case $opt in
		...
		s)	sflag=1
			syscall="$OPTARG" ;;
		\?)	usage; exit 1 ;;	
		:)	printf "ERROR: nebyl specifikovan nazev syscall.\n" >&2; exit 1 ;;
	esac
done
shift $(($OPTIND-1))
